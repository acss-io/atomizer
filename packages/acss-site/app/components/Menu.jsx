    ...
    render: function () {
        ...
        this.props.config.forEach(function (menuitem) {
            if (menuitem.category) {
                menu.push(<h3 className="Fz-14 Bd-t Pt-20" key={menuitem.category}>{menuitem.category}</h3>);
            }
            ...
            if (submenu.length) {
                menu.push(<ul className="Reset NoLinkColor" key={menuitem.category + 'sub'}>{submenu}</ul>);
            }
        });
        return (
