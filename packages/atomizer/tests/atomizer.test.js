...
describe('Atomizer()', () => {
    ...
    describe('findClassNames()', () => {
        ...
        it("returns an array of valid atomic class names even if there's no boundary character for the first found classname", () => {
            const atomizer = new Atomizer();
            const result = atomizer.findClassNames('Pos(r) Ov(h) H(0)');
            const expected = ['Pos(r)', 'Ov(h)', 'H(0)'];
            expect(result).toEqual(expected);
        });
        it('properly resolves custom classnames with hyphen, in custom rules.js', () => {
            const rules = [
                {
                    type: 'pattern',
                    name: 'Font Size',
        ...
        it('properly finds Atomic classnames inside Dust template conditionals', () => {
            const atomizer = new Atomizer();
    ...
    describe('addRules()', () => {
        it('throws if a rule with a different definition already exists', () => {
            const rules = [
                {
                    type: 'pattern',
                    name: 'Border',
        ...
        it("doesn't throw if a rule with the same definition already exists", () => {
            const rules = [
                {
                    type: 'pattern',
                    name: 'Border',
        ...
        it('adds a new rule to the atomizer instance and resets the syntax', () => {
            const atomizer = new Atomizer();
            const myRules = [
                {
                    type: 'pattern',
    ...
    describe('parseConfig()', () => {
        it('returns the expected parsed tree given a config with no options', () => {
            const atomizer = new Atomizer();
            const expected = {
                C: [
                    {
        ...
        it('returns the expected parsed tree given a config with the exclude key', () => {
            const atomizer = new Atomizer();
            const expected = {
                Fl: [
                    {
            ...
            const result = atomizer.parseConfig({
                classNames: ['Fl(end)', 'Fl(start)'],
                exclude: ['Fl(end)'],
            });
            expect(result).toEqual(expected);
        });
        it('returns empty object if invalid class names have been passed', () => {
            const atomizer = new Atomizer();
            const expected = {};
            const result = atomizer.parseConfig({
                classNames: ['RandomInvalidClass'],
            });
            expect(result).toEqual(expected);
        });
        ...
        it('returns proper object with null declarations if invalid class value have been passed', () => {
            const atomizer = new Atomizer();
            const expected = {
                End: [
                    {
    ...
    describe('getConfig()', () => {
        it('returns a valid config object when given classes and no config', () => {
            const atomizer = new Atomizer();
        ...
        it('returns a valid config object when given classes and existing config', () => {
            const atomizer = new Atomizer();
    ...
    describe('getCss()', () => {
        ...
        it('returns expected css for custom classes with break points (Py)', () => {
            // set rules here so if helper change, we don't fail the test
            const atomizer = new Atomizer();
            const config = {
                breakPoints: {
        ...
        it('returns expected css for custom classes with break points (Px)', () => {
            // set rules here so if helper change, we don't fail the test
            const atomizer = new Atomizer();
            const config = {
                breakPoints: {
        ...
        it('returns expected css for custom classes with break points with missing breakPoints', () => {
            // set rules here so if helper change, we don't fail the test
            const atomizer = new Atomizer();
            const config = {
                breakPoints: {
        ...
        it('returns css if colliding helper and atomic rule is used at the same time', () => {
            const atomizer = new Atomizer();
            const config = {
                custom: {
        ...
        it('returns expected css if media query specificity bump option has been passed', () => {
            const atomizer = new Atomizer();
            const config = {
                breakPoints: {
        ...
        it('returns expected css value declared in custom as prop + value', () => {
            const atomizer = new Atomizer();
            const config = {
                custom: {
        ...
        it('returns expected css value declared in custom when using numeric keys', () => {
            const atomizer = new Atomizer();
            const config = {
                custom: {
                    1: '10px solid #ccc',
            ...
            const expected = ['.Bdt\\(1\\) {', '  border-top: 10px solid #ccc;', '}\n'].join('\n');
            const result = atomizer.getCss(config);
            expect(result).toBe(expected);
        });
        it('returns expected css value declared in custom with variable substitution', () => {
            const atomizer = new Atomizer();
            const config = {
                custom: {
                    '$main-color': '#000000',
        ...
        it('returns expected css value declared in custom with nested variable substitution', () => {
            const atomizer = new Atomizer();
            const config = {
                custom: {
                    black: '#000000',
        ...
        it('avoids infinite loops in custom with nested variable substitution', () => {
            const atomizer = new Atomizer();
            const config = {
                custom: {
                    black: '#{$main-color}',
        ...
        it('returns namespaced css when a namespace is specified in options', () => {
            const atomizer = new Atomizer(null, [
                {
                    type: 'pattern',
                    name: 'color',
        ...
        it('warns the user if an ambiguous class is provided and verbose flag is true', (done) => {
    ...
    describe('css variables', () => {
        it('has basic suppport for css variables', () => {
            ...
            const config = {
                ...
                classNames: ['C(--foo)', 'C(--brand-color)', 'Fz(--font-size)', 'Fz(--small-font-size)--sm'],
            };
            const expected = [
                '.C\\(--foo\\) {',
                '  color: var(--foo);',
        ...
        it('should match multiple css variables', () => {
            const atomizer = new Atomizer();
            const config = {
                classNames: ['Bdsp(--foo,--bar)'],
            };
            const expected = ['.Bdsp\\(--foo\\,--bar\\) {', '  border-spacing: var(--foo) var(--bar);', '}\n'].join(
                '\n'
            );
            const result = atomizer.getCss(config);
            expect(result).toBe(expected);
    ...
    describe('escapeSelector()', () => {
        ...
        it('returns the processed string if passed', () => {
            ...
            expect(Atomizer.escapeSelector('#atomic .selector(50%)::something')).toBe(
                '#atomic .selector\\(50\\%\\)\\:\\:something'
            );
            expect(Atomizer.escapeSelector('#atomic .selector(.50%):.:something')).toBe(
                '#atomic .selector\\(\\.50\\%\\)\\:\\.\\:something'
            );
    ...
    describe('sortCSS', () => {
        ...
        it('if two same pseudo class name found, sort by alphabetical order', () => {
            const atomizer = new Atomizer();
            const classNames = ['T(0):f', 'C(#fff):f'];
            expect(atomizer.sortCSS(classNames)).toEqual(['C(#fff):f', 'T(0):f']);
        });

        it('class without pseduo class should sort by by alphabetical order', () => {
            const atomizer = new Atomizer();
            const classNames = [
                'active_D(f)',
                'active_D(n)',
